...
if (!$hasConfig && $currentStep === 1 && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Stacca webhook residuo se c'è un token vecchio nella sessione o config precedente
    // (impedisce che Telegram invii dati durante la reinstallazione)
    // Non possiamo fare nulla senza token — l'utente dovrà aspettare che il vecchio webhook scada (max 60s)
}
...
// FIX: Se già installato e non c'è richiesta di reset/visualizzazione status, redirect al sito
    ...
    <div class="card">
        ...
        <?php elseif ($step === 4): ?>
            ...
            <form method="POST">
                ...
                <div id="caps-msg" style="font-size:12px;color:#f59e0b;margin-top:6px;display:none">⚠️ CAPS LOCK attivo</div>
                <div class="step-nav">
        ...
        <?php elseif ($step === 5): ?>
            <div style="text-align: center;">
                <?php if (isset($_SESSION['install_success'])): ?>
                    <h2 style="color:var(--success)">🎉 Installazione Completata!</h2>

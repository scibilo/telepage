 ...
 * Gestione Contenuti: Editing manuale di titoli, descrizioni e tag.
 */

require_once __DIR__ . '/_auth.php';
adminHeader('Gestione Contenuti', 'contents');
?>

<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
        <div>
            <h2 class="card-title">Archivio Contenuti</h2>
            <p class="card-subtitle">Modifica metadati e catalogazione dei post importati.</p>
        </div>
        <div style="display:flex; gap:10px;">
            <input type="text" id="admin-search" class="form-control" placeholder="Cerca contenuti..." style="width:250px;" oninput="debounceLoad()">
        </div>
    </div>
    ...
    <table class="table" id="contents-table">
        ...
        <tbody id="contents-body">
            <tr><td colspan="5" style="text-align:center; padding:40px;">Caricamento in corso...</td></tr>
        </tbody>
    </table>
...
<div id="edit-modal" class="modal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.8); z-index:1000; align-items:center; justify-content:center;">
    <div class="card" style="width:100%; max-width:800px; max-height:90vh; overflow-y:auto; margin:20px;">
        <h3 id="modal-title" style="margin-bottom:20px;">Modifica Contenuto</h3>
        <form id="edit-form">
            ...
            <div class="form-group">
                <label class="form-label">Descrizione / AI Summary</label>
            ...
            <div class="form-group">
                ...
                <small class="form-hint">Seleziona i tag per catalogare questo contenuto.</small>
            </div>
...
async function loadContents(page = 1) {
    ...
    try {
        ...
        const json = await res.json();
        const data = json.data || json; // supporta sia {ok,data:{...}} che risposta diretta
        const body = document.getElementById('contents-body');
        body.innerHTML = '';

        if (!data.items || data.items.length === 0) {
            body.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:40px;">Nessun contenuto trovato.</td></tr>';
            return;
        }
        ...
        data.items.forEach(item => {
            ...
            tr.innerHTML = `
                <td>
                    ...
                    <a href="${item.url}" target="_blank" style="font-size:12px; color:var(--accent); text-decoration:none;">Vedi sorgente ↗</a>
                </td>
                ...
                <td>
                    <span class="badge ${item.is_deleted ? 'badge-danger' : 'badge-success'}">
                        ${item.is_deleted ? 'ELIMINATO' : 'ATTIVO'}
                    </span>
                </td>
                <td style="text-align: right; white-space:nowrap;">
                    <button class="btn btn-outline" style="padding:4px 8px; font-size:12px;" onclick="openEditModal(${item.id})">Modifica</button>
                    ${item.is_deleted ? 
                        `<button class="btn btn-outline" style="padding:4px 8px; font-size:12px;" onclick="restoreItem(${item.id})">Ripristina</button>` :
                        `<button class="btn btn-outline" style="padding:4px 8px; font-size:12px; color:var(--error);" onclick="deleteItem(${item.id})">Elimina</button>`
                    }
                </td>
            `;
            body.appendChild(tr);
...
function deleteItem(id) {
    if (!confirm('Spostare questo contenuto nel cestino?')) return;

 ...
 * Gestione webhook Telegram real-time.
 *
 * Regole critiche (TELEPAGE_AGENT.md):
 *  RB-01: risponde a Telegram entro 5 secondi
 *  RB-02: valida X-Telegram-Bot-Api-Secret-Token (fatto in api/webhook.php)
 *  RB-06: media salvati con nome hash
 *  RB-07: AI non blocca mai il webhook
 *  RB-08: soft delete (is_deleted=1)
 *  RB-11: upsert su telegram_message_id
 *
 * Flusso (sezione 5.1 architettura):
 *  1. Ricevi update JSON → già validato da api/webhook.php
 *  2. Estrai channel_post o edited_channel_post
 *  3. Verifica chat_id
 *  4. Rileva tipo contenuto
 *  5. Estrai hashtag → tag manuali
 *  6. Scraper::fetch() se URL esterno
 *  7. Salva record in DB (upsert su telegram_message_id)
 *  8. Salva tag manuali
 *  9. Se ai_auto_tag → metti in coda AI (ai_processed=0)
 * 10. Risponde 200 OK (fatto da api/webhook.php)
 */
...
class TelegramBot
{
    ...
    /**
     * Elabora un update Telegram già decodificato da JSON.
     *
    ...
    {
        // Estrai il post del canale (nuovo o modificato)
        $post = $update['channel_post']
             ?? $update['edited_channel_post']
             ?? null;

        if ($post === null) {
            // Update non riguarda un post di canale — ignora silenziosamente
            return null;
        }

        // Verifica che il canale corrisponda a quello configurato (RB-02 logic)
        $config    = Config::get();
    ...
    private static function processPost(array $post, array $config): ?int
    {
        ...
        // Testo del messaggio (può avere caption per media)
        $rawText = $post['text'] ?? $post['caption'] ?? '';

        // --- Rileva tipo contenuto ed estrai URL ---
        [$contentType, $url, $mediaSaved] = self::detectAndSaveMedia($post, $config);

        // Se non c'è URL e non è un media, estrai l'URL dal testo
        if (empty($url)) {
            $url = self::extractUrl($rawText, $post['entities'] ?? $post['caption_entities'] ?? []);
        }

        // --- Estrai hashtag → tag manuali ---
        ...
        if (!empty($url)) {
            // Aggiorna content_type solo se ancora generico
        ...
        // --- Salva nel DB (upsert su telegram_message_id) ---
        ...
        // (già impostato con ai_processed=0 nell'upsert)
    ...
    // -----------------------------------------------------------------------
    // Utilità testo / hashtag
    // -----------------------------------------------------------------------

    /** Estrae tutti gli hashtag da testo e entità Telegram. */
    ...
    /** Estrae il primo URL da entità Telegram o dal testo grezzo. */
    private static function extractUrl(string $text, array $entities): string
    {
        // Prima: entità esplicite (type=url)

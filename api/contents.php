 ...
 * API pubblica contenuti. Nessuna autenticazione.
 * Rate limiting: 60 req/min per IP (sezione 6.1 architettura).
 *
 * GET /api/contents.php
 * Parametri: page, q, tag, type, from, to, per_page, semantic
 */
...
header('Access-Control-Allow-Origin: *'); // API pubblica
...
// -----------------------------------------------------------------------
// Rate limiting: 60 req/min per IP (Sezione 10)
// -----------------------------------------------------------------------
...
if (!checkPublicRateLimit($ip)) {
    ...
    echo json_encode([
        'ok'    => false,
        'error' => 'Rate limit superato. Max 60 richieste/minuto.',
    ]);
    exit;
}
...
// Ricerca full-text su title, description, ai_summary
if ($search !== '') {
...
// -----------------------------------------------------------------------
// Ricerca semantica AI — fallback se < 3 risultati (RB-10)
// -----------------------------------------------------------------------

$semanticUsed = false;
if ($semantic && $search !== '' && $total < 3) {
    // Fase 2: AIService::searchSemantic() — placeholder per ora
    // In Fase 2 verrà implementato con Gemini
    $semanticUsed = false;
}
